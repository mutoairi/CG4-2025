...
/// 静的メンバ変数の実体
/// </summary>
const char* ModelProject::kBaseDirectory = "Resources/";
const char* ModelProject::kDefaultModelName = "cube";
ModelCommonProject* ModelCommonProject::sInstance_ = nullptr;

void ModelProject::StaticInitialize() { ModelCommonProject::GetInstance()->Initialize(); }

void ModelProject::StaticFinalize() { ModelCommonProject::GetInstance()->Terminate(); }

ModelProject* ModelProject::Create() {
	// メモリ確保
...
ModelProject* ModelProject::CreateFromOBJ(const std::string& modelname, bool smoothing) {
	// メモリ確保
	ModelProject* instance = new ModelProject;
...
ModelProject* ModelProject::CreateSphere(uint32_t divisionVertial, uint32_t divisionHorizontal) {
	// メモリ確保
	ModelProject* instance = new ModelProject;
...
void ModelProject::InitializeFromFile(const std::string& modelname, bool smoothing) {
	// モデル読み込み
...
void ModelProject::InitializeFromVertices(const std::vector<Mesh::VertexPosNormalUv>& vertices, const std::vector<uint32_t>& indices) {
...
void ModelProject::LoadModel(const std::string& modelname, bool smoothing) {
...
void ModelProject::LoadMaterial(const std::string& directoryPath, const std::string& filename) {
	// ファイルストリーム
	std::ifstream file;
	// マテリアルファイルを開く
...
void ModelProject::Draw(const WorldTransform& worldTransform, const Camera& camera, const ObjectColor* objectColor) {

	ModelCommonProject* common = ModelCommonProject::GetInstance();
...
void ModelProject::Draw(const WorldTransform& worldTransform, const Camera& camera, uint32_t textureHadle, const ObjectColor* objectColor) {
...
void ModelProject::SetAlpha(float alpha) {
...
ModelCommonProject* ModelCommonProject::GetInstance() {
	if (!ModelCommonProject::sInstance_) {
		ModelCommonProject::sInstance_ = new ModelCommonProject();
	}
	return ModelCommonProject::sInstance_;
}

void ModelCommonProject::Terminate() {
	delete sInstance_;
	sInstance_ = nullptr;
}

void ModelCommonProject::Initialize() {
	// パイプライン初期化
...
void ModelCommonProject::LightCommand(const LightGroup* lightGroup) {
	// ライトコマンドを積む
	if (lightGroup) {
		lightGroup->Draw(commandList_, static_cast<UINT>(ModelProject::RoomParameter::kLight));
	} else {
		defaultLightGroup_->Draw(commandList_, static_cast<UINT>(ModelProject::RoomParameter::kLight));
	}
}

void ModelCommonProject::TransformCommand(const WorldTransform& worldTransform, const Camera& camera) {
	// CBVをセット（ワールド行列）
	commandList_->SetGraphicsRootConstantBufferView(static_cast<UINT>(ModelProject::RoomParameter::kWorldTransform), worldTransform.GetConstBuffer()->GetGPUVirtualAddress());

	// CBVをセット（カメラ）
	commandList_->SetGraphicsRootConstantBufferView(static_cast<UINT>(ModelProject::RoomParameter::kCamera), camera.GetConstBuffer()->GetGPUVirtualAddress());
}

void ModelCommonProject::PreDraw(ID3D12GraphicsCommandList* commandList) {
...
void ModelCommonProject::PostDraw() {
	// コマンドリストを解除
	commandList_ = nullptr;
}

void ModelCommonProject::InitializeGraphicsPipeline() {
